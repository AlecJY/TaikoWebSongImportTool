...
fun loadTJAs(source: Path, category: Genre = Genre.None, similarFile: Boolean = false): ArrayList<TaikoWebSong> {
    val songs = ArrayList<TaikoWebSong>();
    loadTJAs(source, songs, category, similarFile);
    songs.sort();
    return songs;
}

private fun loadTJAs(source: Path, songs: ArrayList<TaikoWebSong>, category: Genre = Genre.None, similarFile: Boolean = false) {
    ...
    for (file in files) {
        if (file.isDirectory) {
            loadTJAs(file.toPath(), songs, genre, similarFile);
